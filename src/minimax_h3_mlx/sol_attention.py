"""Experimental fused Sol-style sparse attention for MiniMax H3 on MLX.
...
class SolAttentionConfig:
    """Runtime policy for the experimental H3 sparse-attention backend."""
...
def _kernel(prefix_blocks: int, summary_blocks: int):
    key = (prefix_blocks, summary_blocks)
...
"""
    kernel = mx.fast.metal_kernel(
        name=f"wee_todd_sol_attention_p{prefix_blocks}_n{summary_blocks}",
...
def sol_attention(
    ...
    *,
    ...
    threadgroup = (32, 4, 1)
    physical = _kernel(prefix_blocks, blocks)(
