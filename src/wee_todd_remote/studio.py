...
_REMOTE_SETTINGS = frozenset(
    {"steps", "guidanceScale", "strength", "shift", "sampler", "numFrames", "fps"}
)
...
def compose_drawthings_request(
    ...
    *,
    ...
    One verified first-frame image may be resolved from Studio assets.
    """
    ...
    fps = _fps(
        configuration.get(
            "fps", override_fps if override_fps is not None else project_settings.get("fps")
        )
    )
    ...
    if "numFrames" in configuration:
        ...
        configuration["numFrames"] = _ltx_frames(duration, fps)
    configuration.setdefault("steps", 8)
